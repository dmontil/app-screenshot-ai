...
export class FixtureAdapter implements ModelProviderPort {
  async generateObject(params: { model?: string; task: string; input: unknown }): Promise<unknown> {
    ...
    if (params.task === "style-reference.analyze") return fixtureStyleReferenceAnalysis(input.app?.appName ?? "fixture-reference");
  ...
  async generateImage(): Promise<{ bytes: Uint8Array; contentType: "image/png" }> {
    return {
      bytes: base64ToBytes("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGN49+4dAAX+Av7czF8cAAAAAElFTkSuQmCC"),
...
function fixtureStyleReferenceAnalysis(referenceId: string) {
  return {
    referenceId,
    visualSummary: "Premium app-store poster system with strong headline area, clean screenshot stage, saturated lighting, and polished depth.",
    layoutRhythm: ["top headline safe zone", "central screenshot stage", "decorative edges", "consistent campaign rhythm"],
    typographyStyle: ["bold short display copy", "high-contrast overlay zone"],
    colorAndLighting: ["soft gradient lighting", "premium glow", "deep shadow separation"],
    compositionRules: ["stage only", "renderer owns readable text and real app UI", "keep screenshot area clear"],
    forbiddenCarryovers: ["fake text", "logos", "trademarks", "fake UI", "phones generated by image model"],
  };
}
